...
<div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start gap-4 mb-6 border-b border-gray-200/50 dark:border-white/10 pb-6">
            <div>
                ...
                <h2 class="text-3xl font-black text-slate-800 dark:text-white tracking-tight flex items-center gap-3">
                    ...
                    <span class="pill {{ $proveedor->tipo_entidad === 'empresa' ? 'pill-done' : 'pill-pending' }} text-sm py-1 px-3 ml-2">
            ...
            <div class="flex flex-wrap gap-3 w-full md:w-auto">
                ...
                <a href="{{ route('inventario.compra.create') }}?proveedor_id={{ $proveedor->id }}" class="btn-primary flex-1 md:flex-none shadow-indigo-500/30 bg-gradient-to-r from-indigo-500 to-blue-500 hover:from-indigo-600 hover:to-blue-600 border-none justify-center">
                    📦 Nueva Compra
                </a>
            </div>
            @endif
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="glass-card p-5 flex flex-col justify-center items-center relative overflow-hidden group border-2 {{ $saldoProveedor > 0 ? 'border-red-500/40 bg-red-500/5 dark:bg-red-900/10' : 'border-teal-500/40 bg-teal-500/5 dark:bg-teal-900/10' }}">
            <div class="absolute -right-6 -top-6 w-24 h-24 {{ $saldoProveedor > 0 ? 'bg-red-500/30' : 'bg-teal-500/30' }} rounded-full blur-2xl group-hover:scale-110 transition-all"></div>
            <p class="text-[10px] font-black {{ $saldoProveedor > 0 ? 'text-red-600 dark:text-red-400' : 'text-teal-700 dark:text-teal-400' }} uppercase tracking-widest mb-1 z-10">Saldo Pendiente</p>
            <p class="text-3xl font-black {{ $saldoProveedor > 0 ? 'text-red-600 dark:text-red-400' : 'text-teal-700 dark:text-teal-400' }} z-10">${{ number_format($saldoProveedor, 0, ',', '.') }}</p>
        </div>
    </div>
