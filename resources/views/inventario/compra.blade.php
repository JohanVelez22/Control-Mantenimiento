        ...
        <form action="{{ route('inventario.compra.store') }}" method="POST" id="compra-form" class="space-y-8">
            ...
            <div>
                <div class="flex justify-between items-center mb-4">
                    ...
                    <button type="button" onclick="agregarFila()" class="btn-ghost text-orange-600 dark:text-orange-400 hover:text-orange-700 hover:bg-orange-500/10 border-orange-500/20">
                        ➕ Agregar artículo
                    </button>
                </div>
            ...
            <div class="flex gap-4 pt-4 border-t border-gray-200/50 dark:border-white/10">
                <button type="submit" class="flex-1 text-center justify-center font-bold text-white bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 py-3 rounded-xl shadow-[0_4px_14px_rgba(249,115,22,0.4)] transition-all transform hover:-translate-y-0.5 border border-orange-400/50">
...
<script>
let filaIndex = 1;
const stocksData = @json($stocks->map(fn($s) => ['id' => $s->id, 'nombre' => $s->producto, 'precio' => $s->precio_compra, 'cantidad' => $s->cantidad]));
