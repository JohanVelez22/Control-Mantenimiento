...
<div class="max-w-5xl mx-auto">
    <div class="glass-card p-6 md:p-8">
        ...
        <form action="{{ route('inventario.venta.store') }}" method="POST" id="venta-form" class="space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 p-5 bg-emerald-50/50 dark:bg-emerald-900/10 border border-emerald-200 dark:border-emerald-500/20 rounded-2xl">
                <div>
            ...
            <div>
                <div class="flex justify-between items-center mb-4">
                    ...
                    <button type="button" onclick="agregarFila()" class="btn-ghost text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 hover:bg-emerald-500/10 border-emerald-500/20">
                        ➕ Agregar artículo
                    </button>
                </div>
            ...
            <div class="flex gap-4 pt-4 border-t border-gray-200/50 dark:border-white/10">
                <button type="submit" class="flex-1 text-center justify-center font-bold text-white bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 py-3 rounded-xl shadow-[0_4px_14px_rgba(16,185,129,0.4)] transition-all transform hover:-translate-y-0.5 border border-emerald-400/50">
...
<script>
let filaIndex = 1;
const stocksData = @json($stocks->map(fn($s) => ['id' => $s->id, 'nombre' => $s->producto, 'precio' => $s->precio_venta, 'cantidad' => $s->cantidad]));
