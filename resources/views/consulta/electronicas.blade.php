        ...
        @elseif($electronicas->isNotEmpty())
            <div class="space-y-3">
                @foreach($electronicas as $e)
                    <a href="{{ route('electronicas.show', $e->id) }}" class="block p-4 glass-card hover:bg-white/50 dark:hover:bg-gray-800/50 transition-colors border-l-4 border-purple-500">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="pill {{ $e->estado === 'terminado' ? 'pill-done' : 'pill-pending' }}">{{ $e->estado === 'terminado' ? '✅' : '⏳' }} {{ ucfirst($e->estado) }}</span>
                        </div>
                    </a>
                @endforeach
            </div>
