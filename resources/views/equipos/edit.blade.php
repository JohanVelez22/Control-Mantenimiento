...
<div class="max-w-4xl mx-auto">
 ...
 <form method="POST" action="{{ route('equipos.update', $equipo->id) }}" class="space-y-6">
 @csrf
 @method('PUT')
 ...
 <input type="text" name="nombre" value="{{ old('nombre', $equipo->nombre) }}" required placeholder="Ej. PC Escritorio" class="glass-input mt-1 @error('nombre') border-red-500 @enderror">
 @error('nombre') <p class="text-red-500 text-xs font-bold mt-1">{{ $message }}</p> @enderror
 </div>

 <div>
 <label class="field-label">Marca *</label>
 <input type="text" name="marca" value="{{ old('marca', $equipo->marca) }}" required placeholder="Ej. HP, Dell" class="glass-input mt-1 @error('marca') border-red-500 @enderror">
 @error('marca') <p class="text-red-500 text-xs font-bold mt-1">{{ $message }}</p> @enderror
 </div>

 <div>
 <label class="field-label">Modelo *</label>
 <input type="text" name="modelo" value="{{ old('modelo', $equipo->modelo) }}" required placeholder="Ej. ProDesk 400" class="glass-input mt-1 @error('modelo') border-red-500 @enderror">
 @error('modelo') <p class="text-red-500 text-xs font-bold mt-1">{{ $message }}</p> @enderror
 </div>

 <div>
 <label class="field-label">Número de Serie *</label>
 <input type="text" name="serie" value="{{ old('serie', $equipo->serie) }}" required placeholder="S/N..." class="glass-input mt-1 font-mono uppercase @error('serie') border-red-500 @enderror">
 ...
 <p class="text-xs text-amber-600 dark:text-amber-400 mt-1">Los equipos inactivos se mostrarán en gris en el listado.</p>
 </div>
 @else
