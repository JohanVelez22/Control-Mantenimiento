...
<div class="max-w-4xl mx-auto">
    <div class="glass-card p-6 md:p-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-6 border-b border-gray-200/50 dark:border-white/10 pb-6 md:pb-8 w-full relative z-20">
            <div class="flex items-center gap-3">
                ...
                @else
                <a href="{{ route('consulta.electronicas') }}" class="btn-ghost px-3 py-2 text-xl" title="Volver a búsqueda">⬅️</a>
                @endif
                <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 text-sm p-5 rounded-2xl bg-purple-50/50 dark:bg-purple-900/10 border border-purple-200 dark:border-purple-500/20">
            <div class="min-w-0">
                <span class="text-[11px] font-bold text-slate-900 dark:text-white uppercase tracking-wider block mb-1">
                    {{ $electronica->equipo?->propietario_tipo === 'proveedor' ? '🏢 Proveedor' : '👤 Cliente' }}
                </span>
                <span class="font-medium text-base text-slate-800 dark:text-slate-200 break-words">{{ $electronica->equipo?->propietario_nombre ?? '-' }}</span>
            </div>
            <div class="min-w-0">
    ...
    <div id="electronica-dynamic-panel" class="transition-opacity duration-150">
        {{-- Resumen financiero --}}
