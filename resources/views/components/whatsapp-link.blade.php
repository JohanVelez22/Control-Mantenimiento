...
@if($url)
    <a href="{{ $url }}" 
       ...
       class="inline-flex items-center gap-1.5 hover:text-emerald-500 dark:hover:text-emerald-400 group transition-colors cursor-pointer {{ $class }}" 
       title="Abrir chat de WhatsApp ({{ $telefono }})">
        @if($showIcon)
            <svg width="17" height="17" viewBox="0 0 24 24" style="width: 17px; height: 17px; min-width: 17px; max-width: 17px; display: inline-block; vertical-align: -2px;" class="shrink-0 group-hover:scale-110 transition-transform">
