        ...
        <div class="flex flex-wrap items-center gap-3">
            <form action="{{ route('stocks.index') }}" method="GET" class="flex gap-2">
                <div class="relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Buscar producto, cod..." class="glass-input pl-9 w-48 sm:w-64">
                </div>
                <button type="submit" class="btn-ghost">Filtrar</button>
            </form>
            @if(!auth()->user()->isInvitado())
                <a href="{{ route('stocks.create') }}" class="btn-primary ml-2 shadow-blue-500/30">
                    ➕ Nuevo Producto
                </a>
            @endif
        </div>
