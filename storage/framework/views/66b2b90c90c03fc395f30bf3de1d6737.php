...
<div class="max-w-4xl mx-auto">
 ...
 <a href="<?php echo e(route('usuarios.index')); ?>" class="btn-ghost px-3 py-2 text-xl" title="Volver">⬅️</a>
 <div>
 <h2 class="text-2xl font-black text-slate-800 dark:text-white tracking-tight">✏️ Editar Usuario: <?php echo e($user->name); ?></h2>
 <p class="text-sm font-medium text-gray-500 dark:text-gray-400 mt-1">Actualiza los datos y credenciales del usuario</p>
 </div>
 </div>

 <form method="POST" action="<?php echo e(route('usuarios.update', $user->id)); ?>" enctype="multipart/form-data">
 <?php echo csrf_field(); ?>
 ...
 <input type="text" name="name" value="<?php echo e(old('name', $user->name)); ?>" required oninput="this.value = this.value.replace(/[^a-zA-ZáéíóúÁÉÍÓÚñÑ ]/g, '')" class="glass-input mt-1 <?php $__errorArgs = ['name'];
...
unset($__errorArgs, $__bag); ?>
 ...
 <input type="email" name="email" value="<?php echo e(old('email', $user->email)); ?>" required class="glass-input mt-1 <?php $__errorArgs = ['email'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> border-red-500 <?php unset($message);
...
unset($__errorArgs, $__bag); ?>
 ...
 <option value="invitado" <?php echo e(old('role', $user->role) == 'invitado' ? 'selected' : ''); ?>>Invitado</option>
 </select>
 </div>
 <?php else: ?>
...
<?php if($user->photo): ?>
  <div class="mb-3">
  <img src="<?php echo e(asset('storage/' . $user->photo)); ?>" width="100" height="100" class="rounded-full object-cover border-2 border-gray-300 dark:border-gray-600 cursor-pointer hover:opacity-80 transition" onclick="openImageLightbox('<?php echo e(asset('storage/' . $user->photo)); ?>', '<?php echo e(addslashes($user->name)); ?>')">
  </div>
  <?php endif; ?>
 <input type="file" name="photo" accept="image/*" class="glass-input <?php $__errorArgs = ['photo'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> border-red-500 <?php unset($message);
...
unset($__errorArgs, $__bag); ?>
  ...
  <input type="password" id="current_password" name="current_password" placeholder="Requerida para cambiar tu contraseña" class="glass-input <?php $__errorArgs = ['current_password'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> border-red-500 <?php unset($message);
...
unset($__errorArgs, $__bag); ?>
 ...
 <input type="password" id="password" name="password" placeholder="Dejar en blanco para no cambiar" class="glass-input <?php $__errorArgs = ['password'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> border-red-500 <?php unset($message);
...
unset($__errorArgs, $__bag); ?>
 ...
 <a href="<?php echo e(route('usuarios.index')); ?>" class="btn-cancel">
 ↩️ Cancelar
 </a>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\ServBay\www\control-mantenimiento-equipos\resources\views/usuarios/edit.blade.php ENDPATH**/ ?>