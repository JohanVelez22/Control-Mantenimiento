...
<div class="space-y-4">
 ...
 <p class="text-2xl font-black text-slate-800 dark:text-white z-10">$<?php echo e(number_format($totales['ingresos'], 0, ',', '.')); ?></p>
 </div>
 ...
 <p class="text-2xl font-black text-slate-800 dark:text-white z-10">$<?php echo e(number_format($totales['egresos'], 0, ',', '.')); ?></p>
 </div>
 ...
 <p class="text-2xl font-black text-slate-800 dark:text-white z-10">$<?php echo e(number_format($totales['efectivo'], 0, ',', '.')); ?></p>
 </div>
 ...
 <p class="text-2xl font-black text-slate-800 dark:text-white z-10">$<?php echo e(number_format($totales['consignacion'], 0, ',', '.')); ?></p>
 </div>
 ...
 <div class="absolute -right-6 -top-6 w-24 h-24 <?php echo e($totales['saldo'] >= 0 ? 'bg-teal-500/20 group-hover:bg-teal-500/30' : 'bg-orange-500/20 group-hover:bg-orange-500/30'); ?> rounded-full blur-2xl transition-all"></div>
 <p class="text-xs font-bold <?php echo e($totales['saldo'] >= 0 ? 'text-teal-600 dark:text-teal-400' : 'text-orange-600 dark:text-orange-400'); ?> uppercase tracking-widest mb-1 z-10 flex items-center gap-1.5 justify-center"><span class="text-lg">⚖️</span> Saldo General</p>
 <p class="text-2xl font-black text-slate-800 dark:text-white z-10">$<?php echo e(number_format($totales['saldo'], 0, ',', '.')); ?></p>
 </div>
 </div>
 ...
 <a href="<?php echo e(route('conceptos.index')); ?>" class="btn-concepts" style="padding: 9px 18px; font-size: 13px;">
 🏷️ <span class="hidden sm:inline">Gestionar Conceptos</span>
 </a>
 <a href="<?php echo e(route('caja.create')); ?>" class="btn-primary flex items-center gap-2 shadow-lg shadow-indigo-500/30" style="padding: 9px 18px; font-size: 13px;">
 ...
 <?php endif; ?>
  ...
  <form action="<?php echo e(route('caja.index')); ?>" method="GET" class="flex flex-wrap items-center gap-3 mb-6 p-5 glass-card no-print relative z-50">
 <div class="relative">
 <span class="absolute z-10 left-3 top-1/2 transform -translate-y-1/2 text-sm select-none pointer-events-none">🔍</span>
 <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Persona o empresa..." class="glass-input pl-9 w-48 sm:w-64 text-sm h-[42px]">
 </div>
 <select name="tipo_movimiento" class="glass-input w-48 text-sm font-semibold no-search h-[42px]">
 <option value="todos" <?php echo e(request('tipo_movimiento') === 'todos' || !request('tipo_movimiento') ? 'selected' : ''); ?>>Todos los tipos</option>
 ...
 <option value="todos" <?php echo e(request('tipo_pago') === 'todos' || !request('tipo_pago') ? 'selected' : ''); ?>>Todos los pagos</option>
 <option value="efectivo" <?php echo e(request('tipo_pago') === 'efectivo' ? 'selected' : ''); ?>>💵 Efectivo</option>
 <option value="consignacion" <?php echo e(request('tipo_pago') === 'consignacion' ? 'selected' : ''); ?>>🏦 Consignación</option>
 </select>
 <div class="flex items-center gap-2">
 <input type="date" name="fecha_desde" value="<?php echo e(request('fecha_desde', date('Y-m-01'))); ?>" class="glass-input w-36 sm:w-44 text-sm h-[42px]">
 <span class="text-gray-400 text-sm">a</span>
 <input type="date" name="fecha_hasta" value="<?php echo e(request('fecha_hasta', date('Y-m-d'))); ?>" class="glass-input w-36 sm:w-44 text-sm h-[42px]">
 </div>
 <button type="submit" class="btn-primary py-2 px-4 text-sm h-[42px]">🌪️ Filtrar</button>
 <a href="<?php echo e(route('caja.index')); ?>" class="btn-clean text-sm h-[42px] flex items-center">🧹 Limpiar</a>
 </form>
...
<td data-label="Código:" class="text-center font-bold <?php echo e($dim); ?>"><?php echo e($m->id); ?></td>
<td data-label="Fecha:" class="text-center font-medium <?php echo e($dim); ?>"><?php echo e($m->fecha->format('d/m/Y')); ?></td>
 <td data-label="Entidad:" class="<?php echo e($dim); ?>">
 <?php if($m->persona): ?>
 <a href="<?php echo e(route('clientes.index', ['search' => $m->persona])); ?>" class="group block hover:opacity-75 transition-opacity" title="Buscar en Clientes">
 ...
 <?php elseif($m->empresa): ?>
 <a href="<?php echo e(route('proveedores.index', ['search' => $m->empresa])); ?>" class="group block hover:opacity-75 transition-opacity" title="Buscar en Proveedores">
 ...
 <td data-label="Concepto:" class="font-medium <?php echo e($dim); ?>">
 <?php echo e($m->concepto->nombre); ?>

 </td>
 <td data-label="Tipo:" class="text-center <?php echo e($dimLight); ?>">
 <span class="pill <?php echo e($m->tipo_movimiento === 'ingreso' ? 'pill-done' : 'pill-egreso'); ?>">
 <?php echo e($m->tipo_movimiento === 'ingreso' ? '📈 Ingreso' : '📉 Egreso'); ?>

 </span>
 </td>
 <td data-label="Pago:" class="text-center <?php echo e($dimLight); ?>">
 <span class="pill <?php echo e($m->tipo_pago === 'efectivo' ? 'pill-efectivo' : 'pill-banco'); ?>">
 <?php echo e($m->tipo_pago === 'efectivo' ? '💵 Efectivo' : '🏦 Banco'); ?>
 ...
 <a href="<?php echo e(route('caja.print', $m->id)); ?>" target="_blank" class="btn-ghost px-3 py-1.5 text-xs" title="Imprimir">🖨️</a>
 <?php if(!auth()->user()->isInvitado()): ?>
 <a href="<?php echo e(route('caja.edit', $m->id)); ?>" class="btn-ghost px-3 py-1.5 text-xs" title="Editar">✏️</a>

                        <button type="button" onclick="openAnularModal('<?php echo e(route('caja.anular', $m->id)); ?>', <?php echo e($m->anulado ? 'true' : 'false'); ?>)" class="btn-ghost px-3 py-1.5 text-xs <?php echo e($m->anulado ? 'grayscale opacity-60 hover:bg-gray-500/10' : 'text-red-600 border-red-500/20 hover:bg-red-500/10'); ?>" title="<?php echo e($m->anulado ? 'Reactivar movimiento' : 'Anular movimiento'); ?>">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\ServBay\www\control-mantenimiento-equipos\resources\views/caja/index.blade.php ENDPATH**/ ?>