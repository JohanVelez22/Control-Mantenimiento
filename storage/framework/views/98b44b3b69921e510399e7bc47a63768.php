...
<div class="space-y-6">
    ...
    <div class="glass-card p-5 relative z-50">
        <form action="<?php echo e(route('eventos.index')); ?>" method="GET" class="flex flex-wrap items-center gap-4">
            <div class="flex items-center gap-2">
                <label class="font-semibold text-sm whitespace-nowrap text-gray-700 dark:text-gray-300">Buscar:</label>
                <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Módulo, observación..." class="glass-input w-48 text-sm">
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                <select name="accion" class="glass-input no-search w-36 text-sm">
                    <option value="todas" <?php echo e(request('accion', 'todas') == 'todas' ? 'selected' : ''); ?>>Ver Todas</option>
                    ...
                    <option value="creado" <?php echo e(request('accion') == 'creado' ? 'selected' : ''); ?>>Creado</option>
                    <option value="actualizado" <?php echo e(request('accion') == 'actualizado' ? 'selected' : ''); ?>>Actualizado</option>
                    <option value="eliminado" <?php echo e(request('accion') == 'eliminado' ? 'selected' : ''); ?>>Eliminado</option>
                    <option value="anulado" <?php echo e(request('accion') == 'anulado' ? 'selected' : ''); ?>>Anulado</option>
                </select>
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                <select name="user_id" class="glass-input w-40 text-sm">
                    <option value="todos" <?php echo e(request('user_id', 'todos') == 'todos' ? 'selected' : ''); ?>>Ver Todos</option>
            ...
            <div class="flex items-center gap-2">
                ...
                <input type="date" name="fecha_desde" value="<?php echo e($fechaDesde); ?>" class="glass-input w-36 text-sm">
            </div>

            <div class="flex items-center gap-2">
                <label class="font-semibold text-sm whitespace-nowrap text-gray-700 dark:text-gray-300">Hasta:</label>
                <input type="date" name="fecha_hasta" value="<?php echo e($fechaHasta); ?>" class="glass-input w-36 text-sm">
            </div>
            ...
            <div class="flex gap-2">
                ...
                <a href="<?php echo e(route('eventos.index')); ?>" class="btn-clean px-4 py-2 flex items-center justify-center font-bold text-sm">
                    🧹 Limpiar
                </a>
            </div>
        </form>
                    ...
                    <?php $__empty_1 = true; $__currentLoopData = $eventos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $evento): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        ...
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                            <td data-label="Fecha:" class="text-xs font-bold text-slate-600 dark:text-slate-300">
                                <?php echo e($evento->created_at->format('d/m/Y h:i A')); ?>

                            </td>
                            <td data-label="Acción:" class="text-center">
                                <span class="inline-flex flex-col items-center justify-center px-2 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider border w-24 leading-tight <?php echo e($badgeClass); ?>">
                                    <span class="text-base mb-0.5"><?php echo e($icon); ?></span>
                                    <span><?php echo e($evento->accion); ?></span>
                                </span>
                            </td>
                            <td data-label="Descripción:" class="font-medium text-sm">
                                <div class="text-slate-800 dark:text-slate-100"><?php echo e($evento->descripcion); ?></div>
                                <?php if($evento->modelo_tipo): ?>
                                    <div class="text-[11px] text-gray-500 font-mono mt-0.5"><?php echo e(class_basename($evento->modelo_tipo)); ?> #<?php echo e($evento->modelo_id); ?></div>
                                <?php endif; ?>
                            </td>
                            <td data-label="Usuario:" class="text-center">
                                <span class="text-sm font-bold text-slate-700 dark:text-slate-300 flex items-center justify-center h-full">
                                    <?php echo e($evento->user->name ?? 'Sistema'); ?>

                                </span>
                            </td>
                            <td data-label="Detalles:" class="text-center">
                                <?php if($evento->valores_antiguos || $evento->valores_nuevos): ?>
                                    <button onclick="openDetalle(<?php echo e($evento->id); ?>)" class="btn-clean px-2 py-1 text-xs font-bold bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 rounded-md transition-colors">
                                        👁️ Ver
                                    </button>
                                    
                                    
                                    <div id="data-ant-<?php echo e($evento->id); ?>" class="hidden"><?php echo json_encode($evento->valores_antiguos, 15, 512) ?></div>
                                    <div id="data-nue-<?php echo e($evento->id); ?>" class="hidden"><?php echo json_encode($evento->valores_nuevos, 15, 512) ?></div>
                                <?php else: ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\ServBay\www\control-mantenimiento-equipos\resources\views/eventos/index.blade.php ENDPATH**/ ?>