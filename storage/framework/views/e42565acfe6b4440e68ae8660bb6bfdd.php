...
<div class="space-y-4">
    ...
    <?php if(!$yaExiste && $preview): ?>
    <div class="glass-card p-6">
        <div class="flex flex-wrap justify-between items-center gap-4 mb-6">
            <div>
                <h2 class="text-xl font-bold flex items-center gap-2"><span class="text-2xl">📅</span> Cierre del Día — <?php echo e(\Carbon\Carbon::parse($hoy)->format('d/m/Y')); ?></h2>
            ...
            <form action="<?php echo e(route('cierre.store')); ?>" method="POST">
                <?php echo csrf_field(); ?>
                <input type="hidden" name="fecha" value="<?php echo e($hoy); ?>">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-5 gap-4">
            <div class="glass-card p-5 flex flex-col justify-center items-center relative overflow-hidden group text-center">
                ...
                <p class="text-2xl font-black text-slate-800 dark:text-white z-10">$<?php echo e(number_format($preview['total_ingresos'], 0, ',', '.')); ?></p>
            </div>
            <div class="glass-card p-5 flex flex-col justify-center items-center relative overflow-hidden group text-center">
                ...
                <p class="text-2xl font-black text-slate-800 dark:text-white z-10">$<?php echo e(number_format($preview['total_egresos'], 0, ',', '.')); ?></p>
            </div>
            <div class="glass-card p-5 flex flex-col justify-center items-center relative overflow-hidden group text-center">
                ...
                <p class="text-2xl font-black text-slate-800 dark:text-white z-10">$<?php echo e(number_format($preview['efectivo'], 0, ',', '.')); ?></p>
            </div>
            <div class="glass-card p-5 flex flex-col justify-center items-center relative overflow-hidden group text-center">
                ...
                <p class="text-2xl font-black text-slate-800 dark:text-white z-10">$<?php echo e(number_format($preview['consignacion'], 0, ',', '.')); ?></p>
            </div>
            <div class="glass-card p-5 flex flex-col justify-center items-center relative overflow-hidden group text-center">
                <div class="absolute -right-6 -top-6 w-24 h-24 <?php echo e($preview['saldo_final'] >= 0 ? 'bg-emerald-500/20 group-hover:bg-emerald-500/30' : 'bg-orange-500/20 group-hover:bg-orange-500/30'); ?> rounded-full blur-2xl transition-all"></div>
                <p class="text-xs font-bold <?php echo e($preview['saldo_final'] >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-orange-600 dark:text-orange-400'); ?> uppercase tracking-widest mb-1 z-10 flex items-center gap-1.5 justify-center">⚖️ Saldo Final</p>
                <p class="text-2xl font-black text-slate-800 dark:text-white z-10">$<?php echo e(number_format($preview['saldo_final'], 0, ',', '.')); ?></p>
            </div>
        </div>
        <p class="text-xs text-gray-500 dark:text-gray-400 font-semibold mt-4 text-right"><?php echo e($preview['num_movimientos']); ?> movimiento(s) registrados hoy</p>
    </div>
    <?php elseif($yaExiste): ?>
    ...
    <div class="glass-card p-6">
        ...
        <div class="overflow-x-auto pb-2">
            <table class="ts-table responsive-table w-full">
                ...
                <tbody>
                    <?php $__empty_1 = true; $__currentLoopData = $cierres; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr>
                        <td data-label="Fecha:" class="text-center font-bold"><?php echo e($c->fecha->format('d/m/Y')); ?></td>
                        <td data-label="Ingresos:" class="text-right text-emerald-600 dark:text-emerald-400 font-bold">$<?php echo e(number_format($c->total_ingresos, 0, ',', '.')); ?></td>
                        <td data-label="Egresos:" class="text-right text-red-600 dark:text-red-400 font-bold">$<?php echo e(number_format($c->total_egresos, 0, ',', '.')); ?></td>
                        <td data-label="Efectivo:" class="text-right text-blue-600 dark:text-blue-400 font-semibold">$<?php echo e(number_format($c->efectivo, 0, ',', '.')); ?></td>
                        <td data-label="Consignación:" class="text-right text-purple-600 dark:text-purple-400 font-semibold">$<?php echo e(number_format($c->consignacion, 0, ',', '.')); ?></td>
                        <td data-label="Saldo:" class="text-right font-black <?php echo e($c->saldo_final >= 0 ? 'text-teal-600 dark:text-teal-400' : 'text-orange-600 dark:text-orange-400'); ?>">$<?php echo e(number_format($c->saldo_final, 0, ',', '.')); ?></td>
                        <td data-label="Mov.:" class="text-center text-sm font-semibold text-gray-500"><?php echo e($c->num_movimientos); ?></td>
                        <td data-label="Registró:" class="text-xs text-gray-500 font-medium"><?php echo e($c->user->name); ?></td>
                        <td data-label="Acciones:" class="text-center">
                            <div class="flex justify-end md:justify-center">
                                <?php if(auth()->user()->isAdmin()): ?>
                                    <button type="button" onclick="openCierrePwd('<?php echo e(route('cierre.destroy', $c->id)); ?>')" class="btn-danger px-3 py-1.5 text-xs" title="Eliminar">🗑️</button>
                                <?php else: ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\ServBay\www\control-mantenimiento-equipos\resources\views/cierre/index.blade.php ENDPATH**/ ?>