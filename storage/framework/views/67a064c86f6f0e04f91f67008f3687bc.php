...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1 space-y-6">
            <?php if(!$movimiento->parent_id && $movimiento->monto_total > 0): ?>
                ...
                <div class="glass-card p-6 relative overflow-hidden group">
                    ...
                    <div class="space-y-3 text-sm z-10 relative">
                        <div class="flex justify-between border-b border-gray-100 dark:border-white/5 pb-2">
                            ...
                            <span class="font-bold text-slate-800 dark:text-white">$<?php echo e(number_format($movimiento->monto_total, 0, ',', '.')); ?></span>
                        </div>
                        <div class="flex justify-between border-b border-gray-100 dark:border-white/5 pb-2">
                            <span class="text-gray-500">Monto Inicial Pagado:</span>
                            <span class="font-bold text-emerald-600 dark:text-emerald-400">$<?php echo e(number_format($movimiento->monto, 0, ',', '.')); ?></span>
                        </div>
                        <div class="flex justify-between border-b border-gray-100 dark:border-white/5 pb-2">
                            <span class="text-gray-500">Total en Abonos:</span>
                            <span class="font-bold text-blue-600 dark:text-blue-400">$<?php echo e(number_format($movimiento->childPayments->where('anulado', false)->sum('monto'), 0, ',', '.')); ?></span>
                        </div>
                        <div class="flex justify-between pt-1">
                            <span class="text-slate-800 dark:text-white font-bold">Saldo Pendiente:</span>
                            <span class="font-black text-lg text-orange-600 dark:text-orange-400">$<?php echo e(number_format($movimiento->saldo_pendiente, 0, ',', '.')); ?></span>
                        </div>
                    </div>
                </div>

                
                <?php if($movimiento->saldo_pendiente > 0): ?>
                    <div class="glass-card p-6">
                        <h3 class="text-lg font-black text-slate-800 dark:text-white mb-4 flex items-center gap-2">💵 Registrar Pago / Abono</h3>
                        
                        <form action="<?php echo e(route('caja.abonos.store', $movimiento->id)); ?>" method="POST" class="space-y-4">
                            <?php echo csrf_field(); ?>
                            <div>
                ...
                <div class="glass-card p-6">
                    ...
                    <?php if($movimiento->childPayments->count() > 0): ?>
                        <div class="space-y-3 max-h-60 overflow-y-auto pr-1">
                            <?php $__currentLoopData = $movimiento->childPayments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $child): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/40 border border-gray-100 dark:border-white/5 relative <?php echo e($child->anulado ? 'opacity-50 grayscale' : ''); ?>">
                                    <div class="flex justify-between items-start mb-1">
                                        <span class="text-xs font-bold text-slate-500"><?php echo e($child->fecha->format('d/m/Y')); ?></span>
                                        <span class="font-black text-sm text-blue-600 dark:text-blue-400">$<?php echo e(number_format($child->monto, 0, ',', '.')); ?></span>
                                    </div>
                                    <p class="text-[11px] text-gray-600 dark:text-gray-400 leading-snug"><?php echo e($child->descripcion); ?></p>
                                    <div class="flex justify-between items-center mt-2 pt-2 border-t border-gray-100 dark:border-white/5 text-[9px] text-gray-400">
                                        <span>Pago: <?php echo e($child->tipo_pago === 'efectivo' ? '💵 Efectivo' : '🏦 Banco'); ?></span>
                                        <span>Registró: <?php echo e($child->user->name ?? 'Sistema'); ?></span>
                                    </div>
                                    <?php if($child->anulado): ?>
            ...
            <?php else: ?>
                ...
                <?php if($movimiento->parent_id): ?>
                    <div class="glass-card p-6 bg-blue-500/5 border border-blue-500/10 text-center">
                        ...
                        <a href="<?php echo e(route('caja.edit', $movimiento->parent_id)); ?>" class="btn-ghost text-xs px-3 py-1.5 mt-3 inline-block font-bold">
                            👁️ Ver Movimiento Padre (#<?php echo e($movimiento->parent_id); ?>)
                        </a>
                    </div>
                <?php endif; ?>
            <?php endif; ?>
