<div class="grid grid-cols-1 md:grid-cols-2 gap-5">
 ...
 <input type="text" name="empresa" id="caja_empresa" value="<?php echo e(old('empresa', $movimiento->empresa ?? '')); ?>" placeholder="Nombre de la empresa..." class="glass-input">
 </div>
 <div>
 <label class="field-label flex items-center gap-2"><span>👤</span> Persona (Opcional)</label>
 <input type="text" name="persona" id="caja_persona" value="<?php echo e(old('persona', $movimiento->persona ?? '')); ?>" placeholder="Nombre de quien paga/recibe..." class="glass-input">
 <?php $__errorArgs = ['persona'];
...
unset($__errorArgs, $__bag); ?>
 ...
 <input type="date" name="fecha" required value="<?php echo e(old('fecha', isset($movimiento) ? $movimiento->fecha->format('Y-m-d') : date('Y-m-d'))); ?>" class="glass-input">
...
unset($__errorArgs, $__bag); ?>
 ...
 <label class="flex-1 flex justify-center items-center gap-2 p-3 rounded-xl border-2 cursor-pointer transition-all <?php echo e(old('tipo_movimiento', $movimiento->tipo_movimiento ?? '') === 'ingreso' ? 'border-emerald-500 bg-emerald-50/50 dark:bg-emerald-900/20' : 'border-gray-200/50 dark:border-white/10 hover:border-emerald-300 dark:hover:border-emerald-700 bg-white/30 dark:bg-slate-800/30 backdrop-blur-md'); ?>">
 <input type="radio" name="tipo_movimiento" value="ingreso" required id="tipo_ingreso" <?php echo e(old('tipo_movimiento', $movimiento->tipo_movimiento ?? '') === 'ingreso' ? 'checked' : ''); ?> class="accent-emerald-500 w-4 h-4">
 <span class="font-bold <?php echo e(old('tipo_movimiento', $movimiento->tipo_movimiento ?? '') === 'ingreso' ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-600 dark:text-slate-400'); ?>">📈 Ingreso</span>
 </label>
 <label class="flex-1 flex justify-center items-center gap-2 p-3 rounded-xl border-2 cursor-pointer transition-all <?php echo e(old('tipo_movimiento', $movimiento->tipo_movimiento ?? '') === 'egreso' ? 'border-red-500 bg-red-50/50 dark:bg-red-900/20' : 'border-gray-200/50 dark:border-white/10 hover:border-red-300 dark:hover:border-red-700 bg-white/30 dark:bg-slate-800/30 backdrop-blur-md'); ?>">
 <input type="radio" name="tipo_movimiento" value="egreso" id="tipo_egreso" <?php echo e(old('tipo_movimiento', $movimiento->tipo_movimiento ?? '') === 'egreso' ? 'checked' : ''); ?> class="accent-red-500 w-4 h-4">
 <span class="font-bold <?php echo e(old('tipo_movimiento', $movimiento->tipo_movimiento ?? '') === 'egreso' ? 'text-red-700 dark:text-red-400' : 'text-slate-600 dark:text-slate-400'); ?>">📉 Egreso</span>
 </label>
 </div>
...
unset($__errorArgs, $__bag); ?>
 ...
 <div class="flex gap-3 mt-1">
 <label class="flex-1 flex justify-center items-center gap-2 p-3 rounded-xl border-2 cursor-pointer transition-all <?php echo e(old('tipo_pago', $movimiento->tipo_pago ?? '') === 'efectivo' ? 'border-blue-500 bg-blue-50/50 dark:bg-blue-900/20' : 'border-gray-200/50 dark:border-white/10 hover:border-blue-300 dark:hover:border-blue-700 bg-white/30 dark:bg-slate-800/30 backdrop-blur-md'); ?>">
 <input type="radio" name="tipo_pago" value="efectivo" required <?php echo e(old('tipo_pago', $movimiento->tipo_pago ?? '') === 'efectivo' ? 'checked' : ''); ?> class="accent-blue-500 w-4 h-4">
 <span class="font-bold <?php echo e(old('tipo_pago', $movimiento->tipo_pago ?? '') === 'efectivo' ? 'text-blue-700 dark:text-blue-400' : 'text-slate-600 dark:text-slate-400'); ?>">💵 Efectivo</span>
 </label>
 <label class="flex-1 flex justify-center items-center gap-2 p-3 rounded-xl border-2 cursor-pointer transition-all <?php echo e(old('tipo_pago', $movimiento->tipo_pago ?? '') === 'consignacion' ? 'border-purple-500 bg-purple-50/50 dark:bg-purple-900/20' : 'border-gray-200/50 dark:border-white/10 hover:border-purple-300 dark:hover:border-purple-700 bg-white/30 dark:bg-slate-800/30 backdrop-blur-md'); ?>">
 <input type="radio" name="tipo_pago" value="consignacion" <?php echo e(old('tipo_pago', $movimiento->tipo_pago ?? '') === 'consignacion' ? 'checked' : ''); ?> class="accent-purple-500 w-4 h-4">
 <span class="font-bold <?php echo e(old('tipo_pago', $movimiento->tipo_pago ?? '') === 'consignacion' ? 'text-purple-700 dark:text-purple-400' : 'text-slate-600 dark:text-slate-400'); ?>">🏦 Banco</span>
 </label>
 </div>
 <?php $__errorArgs = ['tipo_pago'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> <p class="text-red-500 text-xs mt-1 font-bold"><?php echo e($message); ?></p> <?php unset($message);
...
unset($__errorArgs, $__bag); ?>
 ...
 <input type="text" id="monto_total_visual" value="<?php echo e(old('monto_total', isset($movimiento) && $movimiento->monto_total ? number_format($movimiento->monto_total, 0, ',', '.') : '')); ?>" placeholder="Monto total a pagar/cobrar..." class="glass-input font-bold text-right py-2">
 <input type="hidden" name="monto_total" id="monto_total_real" value="<?php echo e(old('monto_total', $movimiento->monto_total ?? '')); ?>">
...
unset($__errorArgs, $__bag); ?>
 ...
 <input type="text" id="monto_visual" required value="<?php echo e(old('monto', isset($movimiento) ? number_format($movimiento->monto, 0, ',', '.') : '')); ?>" placeholder="Monto pagado..." class="glass-input font-bold text-right py-2">
 <input type="hidden" name="monto" id="monto_real" value="<?php echo e(old('monto', $movimiento->monto ?? '')); ?>">
...
unset($__errorArgs, $__bag); ?>
 ...
 <option value="<?php echo e($c->id); ?>" <?php echo e(old('concepto_id', $movimiento->concepto_id ?? '') == $c->id ? 'selected' : ''); ?>>
 <?php echo e($c->nombre); ?>
...
unset($__errorArgs, $__bag); ?>
 ...
 <textarea name="descripcion" rows="2" placeholder="Detalles adicionales del movimiento..." class="glass-input resize-y"><?php echo e(old('descripcion', $movimiento->descripcion ?? '')); ?></textarea>
 </div>
</div>
...
<script>
 ...
 try {
 const res = await fetch('<?php echo e(route('caja.concepto.store')); ?>', {
 method: 'POST',
 headers: {
...
<?php /**PATH C:\ServBay\www\control-mantenimiento-equipos\resources\views/caja/_form.blade.php ENDPATH**/ ?>