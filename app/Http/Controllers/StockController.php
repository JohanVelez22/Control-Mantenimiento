...
class StockController extends Controller
{
    ...
    public function update(Request $request, Stock $stock)
    {
        ...
        if (! isset($validated['precio_venta'])) {
            $validated['precio_venta'] = 0;
        }
        if (! isset($validated['precio_tecnico'])) {
            $validated['precio_tecnico'] = 0;
        }
    ...
    public function reportes(Request $request)
    {
        ...
        if ($request->has('export') && $request->export === 'pdf') {
            $stocks = $query->get();
            $pdf = Pdf::loadView('stocks.pdf_reportes', compact('stocks'))
        ...
        if ($request->has('export') && $request->export === 'excel') {
            ...
            return Excel::download(
                new StocksExport($stocks),
                'Reporte_Inventario_'.date('Ymd_Hi').'.xlsx'
            );
        }

        $stocks = $query->orderBy('id', 'desc')->paginate(20)->withQueryString();
        $categorias = CategoriaStock::where('tipo', 'categoria')->pluck('nombre')->merge(Stock::select('categoria')->whereNotNull('categoria')->where('categoria', '!=', '')->distinct()->pluck('categoria'))->unique();

        $subcategorias = CategoriaStock::where('tipo', 'subcategoria')->pluck('nombre')->merge(Stock::select('subcategoria')->whereNotNull('subcategoria')->where('subcategoria', '!=', '')->distinct()->pluck('subcategoria'))->unique();

        $proveedores = Proveedor::where('active', 1)->orderBy('nombre_razon_social')->get();
    ...
    {
        $historial = FacturaItem::with('factura.facturable', 'factura.user')
    ...
    {
        $proveedor = $stock->proveedor()->first();
        $pdf = Pdf::loadView('stocks.print', compact('stock', 'proveedor'));
        $pdf->setPaper('a4', 'portrait');

        return $pdf->stream('ficha_stock_'.($stock->codigo ?? $stock->id).'.pdf');
    }

    public function anular(Request $request, Stock $stock)
    {
    ...
    public function darDeBaja(Request $request, Stock $stock, StockService $stockService)
    {
    ...
    public function revertirBaja(Request $request, BajaStock $bajaStock, StockService $stockService)
    {
        if (! $bajaStock->exists && $request->route('bajaStock')) {
            $bajaStock = BajaStock::findOrFail($request->route('bajaStock'));
        }
